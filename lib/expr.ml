...
let rec print_expr = function
  ...
  | If(b, e1, e2) -> "If " ^ print_expr b ^ " then " ^ print_expr e1 ^ " else " ^ print_expr e2
  | Function(varName, varType, body) -> "fn " ^ varName ^ ":" ^ print_l1Type varType ^ " => " ^ print_expr body
  | App(e1, e2) -> print_expr e1 ^ " @ " ^ print_expr e2
  | AppOp(op, e1, e2) -> print_expr e1 ^ " " ^ print_op op ^ " " ^ print_expr e2
  | Let(varName, varType, varDefinition, body) -> "Let " ^ varName ^ ":" ^ print_l1Type varType ^ " => " ^ print_expr varDefinition ^ " in " ^ print_expr body
  | LetRec(fName, t1, t2, innerFName, e1, e2) -> 
      ...
      print_expr e2
  | Var name -> name
  | Tuple(first, second) -> "(" ^ print_expr first ^ ", " ^ print_expr second ^ ")"
