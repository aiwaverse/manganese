...
let rec eval ?(ambient : expr Ambient.t = Ambient.empty) (e : expr) =
  ...
  | If(cond, e1, e2) -> (
      match eval cond ~ambient with
        ...
        | Right e -> Left ("eval failed on If, expected Bool, found " ^ print_expr e)
  ...
  | Function (varName, varType, body) -> Right (Function (varName, varType, body))
      ...
      eval e2 ~ambient:
              (Ambient.add 
                  ...
                  (Function 
                      (innerFName, 
                      t1, 
                      LetRec (fName, t1, t2, innerFName, e1, e1)))
                  ambient))
  ...
  | App (f, arg) -> (
      match eval f ~ambient with 
        ...
        | Right err -> Left ("eval failed on App: expected a Function, found" ^ print_expr err)
        | Left err -> Left ("eval failed on App\n" ^ err))
              ...
              | Right (Number int) -> (
                  let n2 = int in
                    match op with
                      ...
                      | Div -> if n2 != 0 then Right (Number (n1 / n2))
                                          else Left ("eval failed on AppOp: division by zero at " ^ print_expr (AppOp(op, Number n1, Number  n2)))
                      | LT -> Right (Boolean (n1 < n2))
                      | LTE -> Right (Boolean (n1 <= n2))
                      | GT -> Right (Boolean (n1 > n2))
                      | GTE -> Right (Boolean (n1 >= n2))
                      | EQ -> Right (Boolean (n1 == n2))
                      | _ -> Left ("eval failed on AppOp: " ^ print_op op ^ "is not a valid operator"))
                | Right err -> Left ("eval failed on AppOp's second argument: expected a Int, found " ^ print_expr err)
                ...
                | Right Boolean bool -> 
                    let b2 = bool in (
                      match op with 
                        ...
                        | _ -> Left ("eval failed on AppOp: " ^ print_op op ^ "is not a valid operator"))
                  | Right err -> Left ("eval failed on AppOp's second argument: expected an Bool, found " ^ print_expr err)
                  | Left err -> Left ("eval failed on AppOp\n" ^ err))
        | Right err -> Left ("eval failed on AppOp's first argument: expected an Bool or a Int, found " ^ print_expr err)
        | Left err -> Left ("eval failed on AppOp\n" ^ err))
  ...
  | Fst t -> (
      match eval t ~ambient with
        ...
        | Right err -> Left ("eval failed on Fst: expected a Tuple, found " ^ print_expr err)
        | Left err -> Left ("eval failed on Fst\n" ^ err))

  | Snd t -> (
      match eval t ~ambient with
        | Right Tuple(_, e2) -> eval e2 ~ambient
        | Right err -> Left ("eval failed on Snd: expected a Tuple, found " ^ print_expr err)
  ...
  | Head list -> (
      ...
      | Right err -> Left("eval failed on Head: expected a Cons, found " ^ print_expr err)
  ...
  | Tail list -> (
      match eval list ~ambient with
        ...
        | Right err -> Left("eval failed on Tail: expected a Cons, found " ^ print_expr err)
     ...
     match eval e1 ~ambient with 
        ...
        | Right Cons (v, vs) -> 
            eval e3 
              ~ambient:
                (Ambient.add_seq
                  (Seq.cons (x, v) (Seq.return (xs, vs)))
                  ambient)
        | Right err -> Left("eval failed on MatchList: expected a Cons or a Nil, found " ^ print_expr err)
  ...
  | MatchMaybe (e1, e2, x, e3) -> (
    match eval e1 ~ambient with 
        Right (Nil _) -> eval e2 ~ambient
      | Right Just v -> 
            eval e3 ~ambient:(Ambient.add x v ambient)
      | Right err -> Left("eval failed on MatchList: expected a Cons or a Nil, found " ^ print_expr err)
      | Left err -> Left ("eval failed on MatchList\n" ^ err))
  | FromJust e -> Right e
