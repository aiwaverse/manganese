...
let simpleTests =
  [
    ( "simple number" >:: fun _ ->
      assert_equal (Right (Number 2)) (eval (Number 2)) );
    ( "simple boolean - true" >:: fun _ ->
      assert_equal (Right (Boolean true)) (eval (Boolean true)) );
    ( "simple boolean - false" >:: fun _ ->
      assert_equal (Right (Boolean false)) (eval (Boolean false)) );
    ( "function" >:: fun _ ->
      assert_equal
        (Right (Function ("x", Bool, If (Var "x", Boolean true, Boolean false))))
        (eval (Function ("x", Bool, If (Var "x", Boolean true, Boolean false)))) );
    ( "let rec" >:: fun _ ->
      assert_equal (Right (Number 120))
        (eval
           (LetRec
              ( "fat",
                Int,
...
let ifTests =
  [
    ( "if - well written" >:: fun _ ->
      assert_equal (Right (Boolean true))
    ...
    ( "if - poorly written, cond poorly written" >:: fun _ ->
       assert_bool "Failed for if"
        (is_left (eval (If (If (Number 0, Boolean true, Boolean false), Boolean true, Boolean false)))) );
  ]
...
let varTests =
  [
    ( "var - in ambient" >:: fun _ ->
      assert_equal (Right (Number 0))
        (eval (Var "x") ~ambient:(Ambient.add "x" (Number 0) Ambient.empty)) );
    ( "var - not in ambient" >:: fun _ ->
      assert_bool "Failed for var"
        (is_left
           (eval (Var "x") ~ambient:(Ambient.add "y" (Number 0) Ambient.empty)))
    );
  ]
...
let appOpTests =
  [
    ( "operator application, add - well written" >:: fun _ ->
      assert_equal (Right (Number 3)) (eval (AppOp (Add, Number 1, Number 2))) );
    ...
    ( "operator application, sub - well written" >:: fun _ ->
      assert_equal (Right (Number (-1))) (eval (AppOp (Sub, Number 1, Number 2))) );
    ...
    ( "operator application, mul - well written" >:: fun _ ->
      assert_equal (Right (Number 2)) (eval (AppOp (Mul, Number 1, Number 2))) );
    ...
    ( "operator application, div - well written" >:: fun _ ->
      assert_equal (Right (Number 0)) (eval (AppOp (Div, Number 1, Number 2))) );
    ...
    ( "operator application, LT - well written" >:: fun _ ->
      assert_equal (Right (Boolean true)) (eval (AppOp (LT, Number 1, Number 2))) );
    ...
    ( "operator application, LTE - well written" >:: fun _ ->
      assert_equal (Right (Boolean true)) (eval (AppOp (LTE, Number 1, Number 2))) );
    ...
    ( "operator application, GT - well written" >:: fun _ ->
      assert_equal (Right (Boolean false)) (eval (AppOp (GT, Number 1, Number 2))) );
    ...
    ( "operator application, GTE - well written" >:: fun _ ->
      assert_equal (Right (Boolean false)) (eval (AppOp (GTE, Number 1, Number 2))) );
    ...
    ( "operator application, EQ - well written" >:: fun _ ->
      assert_equal (Right (Boolean false)) (eval (AppOp (EQ, Number 1, Number 2))) );
...
let sndTests =
  [
    ( "snd - well written" >:: fun _ ->
      assert_equal (Right (Boolean false))
...
let nilTests =
  [
    ( "nil - well written" >:: fun _ ->
      assert_equal (Right (Nil Int)) (eval (Nil Int)) );
  ]

let consTests =
  [
    ( "cons - well written" >:: fun _ ->
      assert_equal (Right (Cons (Number 1, Nil Int))) (eval (Cons (Number 1, Nil Int))) );
    ( "cons - poorly written" >:: fun _ ->
      assert_bool "failed on cons"
        (is_left (eval (Cons (Var "x", Nil Int)))) );
  ]

let headTests =
  [
    ( "head - well typed" >:: fun _ ->
      assert_equal (Right (Number 1)) (eval (Head (Cons (Number 1, Nil Int)))) );
  ...
  let tailTests =
    [
      ( "tail - well typed" >:: fun _ ->
        assert_equal (Right (Nil Int))
          (eval (Tail (Cons (Number 1, Nil Int)))) );
    ...
    let tests =
      "test suite for eval"
      >::: simpleTests @ ifTests @ tupleTests @ varTests @ appTests
           @ appOpTests @ letTests @ fstTests @ sndTests @ nilTests
